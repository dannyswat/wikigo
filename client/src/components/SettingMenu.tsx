...
export function SettingMenu() {
    ...
    return (
        <div className="relative inline-block">
            <button onClick={() => setIsOpen((p) => !p)}><IconSettings size={24} className="inline transition hover:rotate-180" /></button>
            <div className={'absolute right-0 w-48 py-2 mt-2 z-10 bg-white text-left border border-gray-200 rounded-lg shadow-xl ' + (isOpen ? 'block' : 'hidden')}>
                {isAdmin && <button onClick={() => navigate('/users')} className="w-full box-border px-4 py-2 text-left text-gray-800 hover:bg-gray-200">Users</button>}
