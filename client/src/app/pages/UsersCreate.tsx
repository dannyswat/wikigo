...
export default function UsersCreate() {
    ...
    return <div className="w-full flex flex-col gap-4">
        <section className="flex flex-row items-center">
            <label className="basis-1/4">User Name</label>
            <input className="basis-3/4 border-2 rounded-md p-2" type="text" placeholder="User Name" value={data.username}
                onChange={(e) => setData((prev) => ({ ...prev, username: e.target.value }))} />
        </section>
        <section className="flex flex-row items-center">
            <label className="basis-1/4">Email</label>
            <input className="basis-3/4 border-2 rounded-md p-2" type="email" placeholder="Email" value={data.email}
                onChange={(e) => setData((prev) => ({ ...prev, email: e.target.value }))} />
        </section>
        <section className="flex flex-row items-center">
            <label className="basis-1/4">Password</label>
            <input className="basis-3/4 border-2 rounded-md p-2" type="password" placeholder="Password" value={data.password}
